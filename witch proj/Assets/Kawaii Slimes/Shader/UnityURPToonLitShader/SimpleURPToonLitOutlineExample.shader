...
Shader "SimpleURPToonLitExample(With Outline)"
{
    Properties
    {
        ...
        [Header(Base Color)]
        [MainTexture]_BaseMap("Texture map (Albedo)", 2D) = "white" {}
        [HDR][MainColor]_BaseColor("Base Color", Color) = (1,1,1,1)

        [Header(Alpha)]
        [Toggle(Use Alpha Clipping)]_UseAlphaClipping("Use Alpha Clipping", Float) = 0
        _Cutoff("Alpha Cutoff", Range(0.0, 1.0)) = 0.5

        [Header(Emission)]
        [Toggle]_UseEmission("Use Emission", Float) = 0
        [HDR] _EmissionColor("Color", Color) = (0,0,0)
        _EmissionMulByBaseColor("Multiply By Base Color", Range(0,1)) = 0
        [NoScaleOffset]_EmissionMap("Emission Map", 2D) = "white" {}
        _EmissionMapChannelMask("Channel Mask Map", Vector) = (1,1,1,0)

        [Header(Occlusion)]
        [Toggle]_UseOcclusion("Use Occlusion", Float) = 0
        _OcclusionStrength("Strength", Range(0.0, 1.0)) = 1.0
        [NoScaleOffset]_OcclusionMap("Map", 2D) = "white" {}
        _OcclusionMapChannelMask("Channel Mask Map", Vector) = (1,0,0,0)
        _OcclusionRemapStart("Remap Start", Range(0,1)) = 0
        _OcclusionRemapEnd("Remap End", Range(0,1)) = 1

        [Header(Lighting)]
        _IndirectLightMinColor("Indirect Light Min Color", Color) = (0.1,0.1,0.1,1) // can prevent completely black if lightprobe not baked
        _IndirectLightMultiplier("Indirect Light Multiplier", Range(0,1)) = 1
        _DirectLightMultiplier("Direct Light Multiplier", Range(0,1)) = 1
        _CelShadeMidPoint("Cel Shade Mid Point", Range(-1,1)) = -0.5
        _CelShadeSoftness("Cel Shade Softness", Range(0,1)) = 0.05
        _MainLightIgnoreCelShade("Main Light Ignore Cel Shade", Range(0,1)) = 0
        _AdditionalLightIgnoreCelShade("Additional Light Ignore Cel Shade", Range(0,1)) = 0.9

        [Header(Shadow mapping)]
        _ReceiveShadowMappingAmount("Receive Shadow Amount", Range(0,1)) = 0.65
        _ReceiveShadowMappingPosOffset("Receive Shadow Pos Offset", Float) = 0
        _ShadowMapColor("Shadow Color", Color) = (1,0.825,0.78)

        [Header(Outline)]
        _OutlineWidth("Width (World Space)", Range(0,4)) = 1
        _OutlineColor("Color", Color) = (0.5,0.5,0.5,1)
        _OutlineZOffset("Z Offset (View Space)", Range(0,1)) = 0.0001
        [NoScaleOffset]_OutlineZOffsetMaskTex("Z Offset Mask", 2D) = "black" {}//(black is apply ZOffset)
        _OutlineZOffsetMaskRemapStart("Z Offset Mask Remap Start", Range(0,1)) = 0
        _OutlineZOffsetMaskRemapEnd("Z Offset Mask Remap End", Range(0,1)) = 1
    }
    SubShader
    {       
        Tags 
